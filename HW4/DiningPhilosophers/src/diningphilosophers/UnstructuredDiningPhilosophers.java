...
// UnstructuredDiningPhilosophers : A class which facilitates the Dining 
// Philosophers problem with the aid of unstructured (explicit) locks.
public class UnstructuredDiningPhilosophers {
    ...
    // UnstructuredDiningPhilosophers : A constructor method which initializes
    ...
    // seconds that they can eat for. Note that each fork is a ReentrantLock, as
    // the philosophers will lock them to simulate that they have been picked
    // up.
    UnstructuredDiningPhilosophers(int numPhilos, int maxThink, int maxEat) {
        NUM_PHILOS = numPhilos;
        MAX_THINK = maxThink;
        MAX_EAT = maxEat;
        forks = new ReentrantLock[NUM_PHILOS];
    ...
    private class Philosopher implements Runnable {
        ...
        // the philosopher thinks, then tries to pick up the left fork, picks it
        // up when it is available, then tries to pick up the right fork, picks
        // it up when it is available, then eats, then puts the right fork down,
        // then puts the left fork down. This process repeats until an interrupt
        // is caught.
        @Override
        public void run() {
            ...
            while(true) {
                try {
                    ...
                    if(right.isLocked() && right.isHeldByCurrentThread()) {
                        System.out.println("(" + loop + ") Philosopher " + id 
                                + ": Attempted to release right fork.");
                        right.unlock();
                        System.out.println("(" + loop + ") Philosopher " + id 
                                + ": Released right fork.");
                    }
                    if(left.isLocked() && left.isHeldByCurrentThread()) {
                        System.out.println("(" + loop + ") Philosopher " + id 
    ...
    // dine : A method which facilitates an unstructured dinner for an amount of
    // time in seconds.
    public void dine(int dineTime) {
        System.out.println(NUM_PHILOS + " philosophers will have an "
                + "unstructured dinner for " + dineTime + " seconds.");
